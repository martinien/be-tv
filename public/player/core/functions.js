...
//"http://cs.isen.fr/camera/mjpg/video.mjpg"
/*vlcPlayer2.playlist.add("http://cs.isen.fr/camera/mjpg/video.mjpg");
...
function onMessage( message ) {
  ...
  //Custom Events
  if (message == "[entrance]") toggleEntranceView();
  if (message == "[playlist]") togglePlaylist();
  if (message == "[fireAlarm]") toggleFireAlarm();

}
//custom triggers
function toggleEntranceView() {
  if (entranceview === false) {
    entranceblock.visible = true;
    entranceview = true;
  } else {
    entranceblock.visible = false;
    entranceview = false;
  }
}
...
function toggleFireAlarm() {
  togPause();
  if (firealarmview === false) {
   	firealarmblock.visible = true;
  	firealarmview = true;
  } else {
   	firealarmblock.visible = false;
  	firealarmview = false;
  }
}
